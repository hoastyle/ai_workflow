...
agent_context = coordinator.intercept(
    task_description='修复 wf_04_ask 和 wf_05_code 的 Agent 集成',  # 替换为实际任务描述
...
**目的**: 确保命令在老版本 Claude Code 部署环境中正常工作

**执行时机**: 在 Confidence Check 之后，Phase 1 基础代码开发之前

**为什么需要**: Task 2.6 要求"适配老版本环境"，确保新功能（Explore agent, 并行模式）在老版本部署时能优雅降级
...
- ✅ 支持 → 可使用 Explore agent 和并行 agents
- ❌ 不支持 → 降级为传统 Glob/Grep 工具
```

**检测项 2: MCP 服务器可用性**
```
检查:
- Serena MCP: 用于代码理解和精确定位
  * 可用 → 启用 --serena 增强功能
  * 不可用 → 使用 Grep/Read 替代

- Explore agent: 用于代码库探索
  * 可用 → 启用 Step 1.2 智能探索
  * 不可用 → 直接进入 Step 1.3 标准加载
```

**检测项 3: 基础工具完整性**
```
验证: Glob, Grep, Read, Edit, Write, Bash
- 全部可用 → 正常执行
- 部分缺失 → 警告并提供替代方案
```
...
| 限制 | 影响范围 | 降级方案 | 性能影响 |
|------|---------|---------|---------|
| Task tool 不可用 | Step 1.2, Step 3 Mode B | 跳过 Explore，强制 Mode A | Token +70%, 时间 +15min |
| Serena 不可用 | Step 1.4, 所有 --serena 特性 | 使用 Grep/Read 替代 | 定位准确率 95% → 70% |
| 两者都不可用 | 所有增强功能 | 完全传统模式 | Token +70%, 时间 +20min |
...
execution_config:
  ...
  # 强制模式（基于检测结果）
  force_mode_a: false  # 如果 task_tool 不可用，设为 true
  skip_explore: false   # 如果 Explore agent 不可用，设为 true
  disable_serena: false # 如果 Serena 不可用，设为 true
```

**后续步骤自动适配**:
- **Step 1.1**: 如果 `skip_explore == true`，自动跳过复杂度评估，直接进 Step 1.3
- **Step 1.2**: 如果 `explore_agent_available == false`，跳过此步
- **Step 1.4**: 如果 `disable_serena == true`，跳过 Serena MCP 增强
- **Step 2.1**: 如果 `force_mode_a == true`，只显示 Mode A 选项
- **Step 3**: 如果 `force_mode_a == true`，禁用 Mode B（并行开发）
...
**检测结果**:
- Task tool: ✅ 可用 | ❌ 不可用
- Serena MCP: ✅ 可用 | ⚠️ 部分可用 | ❌ 不可用
- Explore agent: ✅ 可用 | ❌ 不可用

**执行模式**: [完全支持 | 部分支持 | 最小模式]

**限制说明** (如果有):
- ⚠️ Task tool 不可用，将使用传统 Glob/Grep 工具（Token 消耗 +70%）
- ⚠️ Serena MCP 不可用，精确定位功能受限（准确率降至 70%）
- ⚠️ 并行开发模式不可用，将使用顺序模式（时间 +15分钟）

**建议** (如果有限制):
- 💡 建议升级到最新版 Claude Code 以获得完整功能
- 💡 或联系管理员启用 MCP 服务器支持
```
...
**性能提升**: 平均 **2.5-3.0x** 时间节省

**执行模式**:
1. **Read → Analyze → Edit**: 适用于功能实现、重构
2. **Search → Read → Edit**: 适用于 Bug 修复、模式替换

**实施示例** (5个文件添加日志):
```
Wave 1: 并行读取 5 个文件 (~5秒 vs 顺序25秒)
Checkpoint: 分析插入点和统一格式
Wave 2: 并行编辑 5 个文件 (~10秒 vs 顺序50秒)
Final: 验证一致性
性能提升: 75秒 → 15秒 (5x)
```

**详细模式、更多示例和最佳实践**: 参见 [并行执行模式指南](docs/guides/parallel_execution_pattern.md)
