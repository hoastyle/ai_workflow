...
### 文档格式约束 (NEW - 2025-11-23)

**关键规则**: 确保Markdown文档在各种显示环境中都能正确渲染

**禁止使用的格式**:
- ❌ emoji编号作为前缀: `1️⃣:`, `2️⃣:`, `3️⃣:` 等
  * 问题：emoji与冒号组合会导致字符重叠
  * 修复：改用 `Step 1:`, `1.` 或 `### 步骤 1`
  * 示例：`❌ 1️⃣: 第一步` → `✅ Step 1: 第一步`

**推荐的替代方式**:
- 使用纯文本编号：`Step 1`, `Step 2`, `1.`, `2.`
- 使用有序列表：`1.`, `2.`, `3.`
- 使用标题：`### 步骤 1`, `### 步骤 2`
- 使用流程描述：`然后`, `之后`, `下一步` (代替箭头)

**详细规范**: [docs/reference/MARKDOWN_STYLE.md](docs/reference/MARKDOWN_STYLE.md)

**自动验证**: pre-commit钩子会检查Markdown格式违规
