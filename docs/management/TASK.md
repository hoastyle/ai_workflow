...
### 新增任务：wf 命令系统优化 (代码审查 2025-11-15)

#### A. 为所有 wf 命令添加工作流导航部分 (🔴 高优先级)

**基本信息**:
- 影响范围: 10 个 wf 命令文件 (wf_01, wf_02, wf_03, wf_04, wf_04_research, wf_06, wf_12, wf_13, wf_14, wf_99)
- 预计时间: 2-3小时
- 工作流位置: [代码审查] → [代码重构] → [代码审查] → [提交]
- 为什么优先: 完整实现 Phase 3 闭环工作流，确保工作流体验一致性

**推荐命令序列**:
```bash
# 第1步: 标记任务为活跃
/wf_02_task update "为所有 wf 命令添加工作流导航部分"

# 第2步: 代码重构 (主要工作)
/wf_09_refactor "为所有缺少工作流导航的 wf 命令添加此部分"

# 第3步: 代码审查
/wf_08_review

# 第4步: 提交并保存进度
/wf_11_commit "refactor: 统一所有 wf 命令的工作流导航部分"
```

**验收标准**:
- [ ] wf_01_planning.md 添加工作流导航
- [ ] wf_02_task.md 添加工作流导航
- [ ] wf_03_prime.md 添加工作流导航
- [ ] wf_04_ask.md 添加工作流导航
- [ ] wf_04_research.md 添加工作流导航
- [ ] wf_06_debug.md 添加工作流导航
- [ ] wf_12_deploy_check.md 添加工作流导航
- [ ] wf_13_doc_maintain.md 添加工作流导航
- [ ] wf_14_doc.md 添加工作流导航
- [ ] wf_99_help.md 添加工作流导航
- [ ] 所有工作流导航部分格式一致
- [ ] 代码审查通过
- [ ] 提交到仓库
...
#### B. 简化 wf_14_doc.md，提取示例到外部文档 (🔴 高优先级)

**基本信息**:
- 影响范围: wf_14_doc.md (1379 → ~500 行)
- 预计时间: 1.5小时
- 工作流位置: [代码实现] → [代码审查] → [代码重构] → [提交]
- 为什么优先: 遵循"Simplify Ruthlessly"原则，提升文件可维护性

**创建文件**:
- `docs/examples/wf_14_doc_examples.md` - 示例代码 (121 行)
- `docs/examples/documentation_templates.md` - 文档模板 (156 行)

**推荐命令序列**:
```bash
# 第1步: 标记任务为活跃
/wf_02_task update "简化 wf_14_doc.md，提取示例到外部文档"

# 第2步: 代码实现 (主要工作: 提取和重组)
/wf_05_code "提取 wf_14_doc.md 中的示例内容到外部文档"

# 第3步: 代码审查
/wf_08_review

# 第4步: 提交
/wf_11_commit "refactor: 简化 wf_14_doc.md，提取示例到外部文档"
```

**验收标准**:
- [ ] 创建 docs/examples/wf_14_doc_examples.md
- [ ] 创建 docs/examples/documentation_templates.md
- [ ] wf_14_doc.md 中的 Examples 部分替换为指向新文档的链接
- [ ] wf_14_doc.md 行数减少到 500-600 行
- [ ] 所有链接有效
- [ ] 代码审查通过
- [ ] 提交到仓库
...
#### C. 标准化所有 wf 命令的 YAML Frontmatter (🟠 中优先级)

**基本信息**:
- 影响范围: 所有 16 个 wf 命令文件
- 预计时间: 2小时
- 工作流位置: [代码实现] → [代码审查] → [提交]
- 为什么优先: 为命令生成关系图和自动化索引奠定基础

**推荐命令序列**:
```bash
# 第1步: 标记任务为活跃
/wf_02_task update "标准化所有 wf 命令的 YAML Frontmatter"

# 第2步: 代码实现
/wf_05_code "为所有 wf 命令文件添加标准化的 YAML Frontmatter"

# 第3步: 代码审查
/wf_08_review

# 第4步: 提交
/wf_11_commit "refactor: 统一 wf 命令的 Frontmatter 元数据格式"
```

**验收标准**:
- [ ] 所有 16 个文件都有 YAML Frontmatter
- [ ] Frontmatter 包含标准字段 (command, index, phase, description, reads, writes, prev_commands, next_commands)
- [ ] 所有命令关系定义清晰
- [ ] KNOWLEDGE.md 可以自动生成命令依赖图
- [ ] 代码审查通过
- [ ] 提交到仓库

---

#### D. 提取强制语言规则到 CLAUDE.md (🟠 中优先级)

**基本信息**:
- 影响范围: CLAUDE.md, wf_03_prime.md, wf_14_doc.md
- 预计时间: 45分钟
- 工作流位置: [代码实现] → [代码审查] → [提交]
- 为什么优先: 遵循 DRY (Don't Repeat Yourself) 原则，统一语言规范

**推荐命令序列**:
```bash
# 第1步: 标记任务为活跃
/wf_02_task update "提取强制语言规则到 CLAUDE.md"

# 第2步: 代码实现
/wf_05_code "提取强制语言规则到 CLAUDE.md，简化各命令文件"

# 第3步: 代码审查
/wf_08_review

# 第4步: 提交
/wf_11_commit "refactor: 提取语言规则到 CLAUDE.md，遵循 DRY 原则"
```

**验收标准**:
- [ ] CLAUDE.md 的"语言规范"部分包含 AI 输出规则
- [ ] wf_03_prime.md 中的"⚠️ 强制语言规则"部分删除或简化为链接
- [ ] wf_14_doc.md 中的"⚠️ 强制语言规则"部分删除或简化为链接
- [ ] 其他 14 个命令文件中添加链接指向 CLAUDE.md
- [ ] 代码审查通过
- [ ] 提交到仓库

---

#### E. 创建 AI 角色库参考文档 (🟡 低优先级)

**基本信息**:
- 创建文件: `docs/reference/AI_ROLES.md`
- 预计时间: 30分钟
- 工作流位置: [代码实现] → [提交]
- 为什么优先: 为未来的工作流设计和扩展奠定基础

**推荐命令序列**:
```bash
# 第1步: 标记任务为活跃
/wf_02_task update "创建 AI 角色库参考文档"

# 第2步: 代码实现
/wf_05_code "创建 AI 角色库，定义所有工作流中使用的角色"

# 第3步: 提交
/wf_11_commit "docs: 创建 AI 角色库参考文档"
```

**验收标准**:
- [ ] 创建 docs/reference/AI_ROLES.md
- [ ] 定义所有工作流中使用的角色
- [ ] 包含角色描述和职责说明
- [ ] 提交到仓库
