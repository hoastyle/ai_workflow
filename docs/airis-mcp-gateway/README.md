---
title: "AIRIS MCP Gateway 使用指南"
description: "统一访问 13 个 MCP 服务器的完整使用指南"
type: "系统集成"
status: "完成"
priority: "高"
...
**重要说明**: AIRIS MCP Gateway 通过两种方式管理 MCP 服务器:

1. **ProcessManager** (主要): 管理 13 个服务器（通过 npx/uvx 启动）
2. **Docker Gateway** (辅助): 管理 2 个服务器（mindbase + time）通过 Docker 容器

本文档主要介绍 ProcessManager 管理的 13 个服务器。MindBase 和 Time 的详细说明见 [服务器文档](servers/)。
