...
python $HOME/.claude/commands/scripts/doc_guard.py \
  --docs "$HOME/.claude/commands/docs/guides/wf_06_debug_workflows.md" \
  --sections "{\"$HOME/.claude/commands/docs/guides/wf_06_debug_workflows.md\": [\"AI执行协议\", \"模式选择决策树\", \"执行检查清单\"]}"
```
...
- Agent 成功激活（匹配度 ≥ 85%）
- Agent 推荐的命令 ≠ 用户执行的命令
- 例如：用户运行 `/wf_06_debug`，但 Agent 推荐 `/wf_04_ask`
...
**MCP 工具推荐示例（针对 debug-agent）**:

```
Agent 推荐的 MCP 工具：
  🔴 Sequential-thinking (高优先级) - 结构化错误分析和假设验证
  🟠 Serena (中优先级) - 代码级定位和依赖影响分析
```
...
✅ **Sequential-thinking** - 结构化分析错误原因
  用途：通过系统化步骤分析根本原因、生成验证假设、优先级排序

✅ **Serena** - 代码级深度定位
  用途：精确定位错误相关代码、识别相关依赖、影响范围分析
...
| 优先级 | 条件 | 行为 |
|--------|------|------|
| 🔴 高 | Agent 强烈推荐（置信度 > 80%） | 必须启用，不能禁用 |
| 🟠 中 | Agent 推荐（置信度 50-80%） | 默认启用，可选禁用 |
| 🟡 低 | Agent 可选推荐（置信度 < 50%） | 不启用（除非用户选择） |

**检查清单** (Step 0.3 完成验证):
- [ ] ✅ 检测到 agent 的 mcp_hints
- [ ] ✅ 自动启用了高优先级 MCP 工具
- [ ] ✅ 在执行中实际调用了 MCP 工具
- [ ] ✅ 在输出中明确显示了使用的工具和原因
...
### Step 0.4: Confidence Check (Pre-Debugging Assessment) 🎯
