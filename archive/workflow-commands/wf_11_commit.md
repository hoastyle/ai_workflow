...
## Usage
```bash
/wf_11_commit [message] [--quick] [--no-verify]
```

**参数**：
- `message`: 提交消息（必需）
- `--quick`: 快速模式，跳过格式化和分析，只执行核心提交流程（可选）
- `--no-verify`: 跳过 pre-commit hooks 验证（可选，慎用）

**示例**：
```bash
# 完整模式（默认）- 适合重要提交
/wf_11_commit "feat: 添加新功能"

# 快速模式 - 适合小改动
/wf_11_commit "fix: 修复拼写错误" --quick

# 快速模式 + 跳过 hooks - 紧急情况使用
/wf_11_commit "docs: 更新文档" --quick --no-verify
```
...
是否包含 --quick 标志？
    ...
    │     ↓
    │     1. 暂存文件 (git add -A)
    │     2. 生成提交消息（带 Co-Authored-By 签名）
    │     3. 执行提交 (git commit)
    │     4. 更新 CONTEXT.md（最小化版本）
    │     5. 显示完成报告
    │
    否 → 完整模式
          ↓
...
**Step 2：生成提交消息**
```bash
# 提取用户消息（移除 --quick 和 --no-verify 标志）
...
echo "✅ 提交成功: $(git log -1 --oneline)"
```

**Step 4：更新 CONTEXT.md（最小化版本）**
```markdown
# CONTEXT.md
...
### 会话状态
- Git commits (本次会话): 1 commit (快速模式)
- 修改文件数: $(git diff --name-only HEAD~1 | wc -l) files
- 提交消息: $USER_MESSAGE
...
**注意**: 如果用户使用 `--quick` 标志，**跳过此完整流程**，执行上述快速模式流程。
