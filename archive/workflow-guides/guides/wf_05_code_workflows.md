...
Step 0: 读取本执行协议 (当前)
  ↓
Step 1: 任务确认与范围理解（强制执行 遵循TASK.md）
  ↓
...
## Step 1: 任务确认与范围理解 🔴 **[强制执行]**

⚠️ **AI 强制规则**: 此步骤**必须执行**，不得跳过或假设已理解

**执行时机**: 在开始代码实现之前

**目的**: 确保 AI 完整理解任务目标、范围和验收标准，避免范围蔓延或方向错误
...
```bash
# 检查 TASK.md 是否存在
if [ -f "TASK.md" ] || [ -f "docs/management/TASK.md" ]; then
    # 场景 A: 完整工作流项目
    echo "✅ 发现 TASK.md，读取任务详情"
else
    # 场景 B/C: 降级处理
    echo "⚠️ TASK.md 不存在，主动确认任务信息"
fi
```

### 1.2 场景 A: TASK.md 存在的处理

**当用户提及任务编号时** (例如: "任务 9.1 优化依赖管理"):

```markdown
Step 1: 读取 TASK.md
  ↓
定位任务编号（使用 Grep 或 Read 工具）
  ↓
确认任务详情:
  - 任务目标: [从 TASK.md 提取]
  - 工作范围: [从 TASK.md 提取]
...
**任务目标**: 优化 C++ 项目依赖管理
**工作范围**:
- 将手动管理的第三方库迁移到 Git Submodule
- 修改 CMakeLists.txt 构建配置
- 更新开发文档

**验收标准**:
- ✅ 所有第三方库通过 Git Submodule 管理
- ✅ CMake 构建成功且无警告
- ✅ 文档更新完整
...
### 1.3 场景 B/C: TASK.md 不存在的处理

**当用户提及任务但 TASK.md 不存在时**:
...
输出确认请求:
  "我注意到当前项目没有 TASK.md。
   关于任务 [用户提及的任务]，能否确认以下信息:

   1. 任务目标: [AI 根据用户描述理解的目标]
   2. 工作范围: [AI 理解的范围]
   3. 验收标准: [如何判断任务完成]
   4. 是否有已知的限制或约束?
...
我注意到当前项目没有 TASK.md。关于你提到的"任务 9.1 优化 C++ 依赖管理"，
让我确认一下理解:

**我的理解**:
1. **任务目标**: 改进 C++ 项目的第三方库依赖管理方式
2. **工作范围**:
   - 使用 Git Submodule 替代当前的手动管理
...
简短确认即可，无需冗长等待
  ↓
如果用户说"正确"或"开始吧"，进入 Step 2
```

### 1.5 检查清单（Step 1 完成验证）

在进入 Step 2 之前，AI 必须确认:

- [ ] ✅ 已检查 TASK.md 是否存在
- [ ] ✅ 已获取任务的完整信息（目标/范围/验收标准）
- [ ] ✅ 已向用户确认理解（或获得用户明确的"开始"信号）
- [ ] ✅ 理解了任务的约束和限制（如果有）

**如果任何检查项未通过，不得进入 Step 2**
...
- [ ] ✅ 已读取本执行协议
- [ ] ✅ **已完成 Step 1 任务确认** ← 新增强制项
  - 检查了 TASK.md 是否存在
  - 获取了任务的完整信息（目标/范围/验收标准）
  - 向用户确认理解或获得"开始"信号
- [ ] ✅ 已根据决策树选择实现模式并明确说明理由
