...
本命令支持以下 MCP 服务器的增强功能。

### Serena (符号级引用完整性检查)

**启用**: 条件自动激活（检测到符号修改时）
**用途**: 符号级别的引用完整性验证，确保所有调用点已同步更新
**自动激活**: 代码审查中检测到符号修改（类、函数、方法改名或签名变更）
...
# 显式审查符号引用
/wf_08_review "AuthService 类重构"
```

**改进点**:
- 符号级引用完整性检查（find_referencing_symbols）
- 自动发现所有调用点和依赖关系
- 验证签名变更是否同步到所有引用处
- 识别潜在的破坏性变更
- 检测未更新的调用点
...
### Sequential-thinking (结构化审查策略)

**启用**: `--think` 标志（可选）
**用途**: 结构化多步推理分析审查策略
**适用场景**: 复杂架构审查、多维度评估、设计决策分析

**示例**:
```bash
# 启用结构化审查分析
/wf_08_review "大规模重构审查" --think

# 组合使用
/wf_08_review "微服务架构迁移" --think
```
...
if gateway.is_available("serena"):
    ...
    modified_symbol = find_tool.call(
        name_path_pattern="ModifiedClass/modifiedMethod",
    ...
    references = ref_tool.call(
        name_path="ModifiedClass/modifiedMethod",
        relative_path="src/module.py"
    )

    # Step 3: 验证所有引用点是否已同步更新
    ...
    print("⚠️ Serena MCP 不可用，使用手动引用检查")
```
...
if gateway.is_available("sequential-thinking"):
    ...
    print("⚠️ Sequential-thinking 不可用，使用标准审查流程")
```

**组合使用示例** (Serena + Sequential-thinking):
```python
# 初始化 Gateway
gateway = get_mcp_gateway()
